...
from aigo.utils import coords_from_point
...
class MCTS(object):
    ...
    def add_random_child(self):
        ...
        new_move = self.unvisited_moves.pop(i)
        new_game_state = self.game_state.apply_move(new_move)
        new_node = MCTS(new_game_state, self, new_move)
...
class MCTSAgent(agent.Agent):
    ...
    def select_move(self, game_state):
        root = MCTS(game_state)

        for i in range(self.num_rounds):
            node = root 
            while(not node.add_child()) and (not node.is_terminal()):
    ...
    def select_child(self, node):
        total_rollouts = sum(child.sum_rollouts for child in node.children)
        ...
        for child in node.children: 
            win_percent = child.winning_frac(node.game_state.next_player)
            explor_factor = math.sqrt(log_rollouts / child.num_rollouts)
            uct_score = win_percent + self.temp * explor_factor
